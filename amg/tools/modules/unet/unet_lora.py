...
class MemoryEfficientCrossAttention(nn.Module):
    ...
    def __init__(self, query_dim, context_dim=None, heads=8, dim_head=64, dropout=0.0,
        ...
        if use_lora:
            ...
            self.to_k_lora = LoRA(context_dim, inner_dim,
                                  lora_rank, lora_alpha)
            self.to_v_lora = LoRA(context_dim, inner_dim,
                                  lora_rank, lora_alpha)
            self.to_out_lora = LoRA(
                inner_dim, query_dim, lora_rank, lora_alpha)
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x, context=None):
        ...
        if context is not None:
            ck, cv = self.context_kv(context).reshape(
                b, -1, n * 2, d).permute(0, 2, 3, 1).chunk(2, dim=1)
...
class TemporalAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        if self.use_sim_mask:
            sim_mask = torch.tril(torch.ones(
                (n, n), device=device, dtype=torch.bool), diagonal=0)
    ...
    def forward(self, x, context=None):
        ...
        if self.only_self_att:
            ...
            for i, block in enumerate(self.transformer_blocks):
                ...
                context[i] = rearrange(
                    context[i], '(b f) l con -> b f l con', f=self.frames).contiguous()
                # calculate each batch one by one (since number in shape could not greater then 65,535 for some package)
                for j in range(b):
                    context_i_j = repeat(
                        context[i][j], 'f l con -> (f r) l con', r=(h*w)//self.frames, f=self.frames).contiguous()
        ...
        if not self.use_linear:
            ...
            x = self.proj_out(x)
            x = rearrange(x, '(b h w) c f -> b c f h w',
                          b=b, h=h, w=w).contiguous()
...
class UNetSD_LoRA(nn.Module):
    def __init__(self,
        ...
        disabled_sa = False
        # params
        # [320, 320, 640, 1280, 1280]
        enc_dims = [dim * u for u in [1] + dim_mult]
        dec_dims = [dim * u for u in [dim_mult[-1]] +
                    dim_mult[::-1]]  # [1280, 1280, 1280, 640, 320]
        shortcut_dims = []
        scale = 1.0
        ...
        if temporal_attention:
            if USE_TEMPORAL_TRANSFORMER:
                ...
                init_block.append(TemporalAttentionMultiBlock(dim, num_heads, head_dim, rotary_emb=self.rotary_emb,
                                  temporal_attn_times=temporal_attn_times, use_image_dataset=use_image_dataset))
        # elif temporal_conv:
        ...
        for i, (in_dim, out_dim) in enumerate(zip(enc_dims[:-1], enc_dims[1:])):
            for j in range(num_res_blocks):
                block = nn.ModuleList([ResBlock(in_dim, embed_dim, dropout, out_channels=out_dim,
                                      use_scale_shift_norm=False, use_image_dataset=use_image_dataset,)])
                if scale in attn_scales:
                    ...
                    if self.temporal_attention:
                        if USE_TEMPORAL_TRANSFORMER:
                            ...
                            block.append(TemporalAttentionMultiBlock(out_dim, num_heads, head_dim, rotary_emb=self.rotary_emb,
                                         use_image_dataset=use_image_dataset, use_sim_mask=use_sim_mask, temporal_attn_times=temporal_attn_times))
                in_dim = out_dim
        ...
        if self.temporal_attention:
            if USE_TEMPORAL_TRANSFORMER:
                ...
                self.middle_block.append(TemporalAttentionMultiBlock(out_dim, num_heads, head_dim, rotary_emb=self.rotary_emb,
                                         use_image_dataset=use_image_dataset, use_sim_mask=use_sim_mask, temporal_attn_times=temporal_attn_times))
        ...
        for i, (in_dim, out_dim) in enumerate(zip(dec_dims[:-1], dec_dims[1:])):
            for j in range(num_res_blocks + 1):
                block = nn.ModuleList([ResBlock(in_dim + shortcut_dims.pop(), embed_dim, dropout,
                                      out_dim, use_scale_shift_norm=False, use_image_dataset=use_image_dataset, )])
                if scale in attn_scales:
                    block.append(
                        SpatialTransformer(
    ...
    def forward(self,
        ...
        if mask_last_frame_num > 0:
            ...
            focus_present_mask = default(focus_present_mask, lambda: prob_mask_like(
                (batch,), prob_focus_present, device=device))  # [False, False]
        ...
        for block in self.input_blocks:
            x = self._forward_single(
                block, x, e, context, time_rel_pos_bias, focus_present_mask, video_mask)
            xs.append(x)
        ...
        for block in self.output_blocks:
            ...
            x = self._forward_single(block, x, e, context, time_rel_pos_bias,
                                     focus_present_mask, video_mask, reference=xs[-1] if len(xs) > 0 else None)
