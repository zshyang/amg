...
@torch.no_grad()
def visualize(cfg, batch, diffusion, model, clip_encoder, caption):
    """
    visualize the model
    """
    ...
    caption_y, _ = encode_conditions(clip_encoder, caption)
...
def decode_to_video(video_data, cfg, autoencoder, save_path):
    ...
    # Close the writer
    writer.close()
    if not cfg.keep_image:
        os.system(f'rm -rf {frame_dir}')


def load_cfg_from_argparse():
    """
    load config based on args
    """
...
def main():
    """
    main function used to change the background of the video
    """
    # load cfg
    ...
    with open(
        os.path.join(
            cfg.new_caption_json_folder_path,
            f"frame_{cfg.vid_dataset.start_frame:06d}.json",
        )
    ) as open_json:
        data = json.load(open_json)
        cfg.new_captions = data["new_captions"]
        cfg.new_caption_file_names = data["new_caption_file_names"]

    for caption, caption_file_name in zip(
        cfg.new_captions, cfg.new_caption_file_names
    ):
        (
            video_data, gh_video_feature_data, video_feature_data,
        ) = visualize(cfg, batch, diffusion, model, clip_encoder, caption)
